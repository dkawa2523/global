    ...
    def reaction_budget(
        ...
        *,
        ...
        out: dict[str, Any] = {
            ...
            'sign_convention': 'positive gas flux is returned to the gas; positive surface term increases coverage',
            'surfaces': {},
        }
        ...
        for surface in sys.chamber.surfaces:
            ...
            for sp_id, idx in sys.state_layout.surface_index.get(surface_id, {}).items():
                species_payload[sp_id] = {
                    'coverage': float(state[idx]),
                    'production_s_inv': 0.0,
                    'loss_s_inv': 0.0,
                    'net_s_inv': 0.0,
                    '_production_terms': [],
                    '_loss_terms': [],
                }
            out['surfaces'][surface_id] = {
                'zone_id': surface.zone_id,
                'area_m2': float(surface.area_m2),
                'site_density_m2': float(surface.site_density_m2),
                'site_metrics': self.surface_site_metrics(surface_id, state),
        ...
        for rxn in self.surface_reactions:
            z = sys.zone_index[rxn.zone_id]
            eval_ = self.surface_rate(rxn, gas[z], float(Tg[z]), state, step, coupled)
            rate = eval_.rate_m2_s
            if rate == 0.0:
                continue
            surface_bucket = out['surfaces'][rxn.surface_id]
            reaction_gas: dict[str, float] = {}
            reaction_surface: dict[str, float] = {}

            for _idx, nu, sp_id in rxn.delta_gas:
                flux = float(nu * rate)
                reaction_gas[sp_id] = flux
                gas_payload = surface_bucket['gas_fluxes_m2_s'].setdefault(
                    sp_id,
                    {'production_to_gas_m2_s': 0.0, 'consumption_from_gas_m2_s': 0.0, 'net_to_gas_m2_s': 0.0},
                )
                gas_payload['net_to_gas_m2_s'] += flux
                if flux >= 0.0:
                    gas_payload['production_to_gas_m2_s'] += flux
                else:
                    gas_payload['consumption_from_gas_m2_s'] += -flux

            for _idx, nu, sp_id in rxn.delta_surface:
                term = float(nu * rate / rxn.site_density_m2)
                reaction_surface[sp_id] = term
                if sp_id not in surface_bucket['species']:
                    continue
                payload = surface_bucket['species'][sp_id]
                payload['net_s_inv'] += term
                ranked = {'process_id': rxn.reaction_id, 'contribution_s_inv': abs(term), 'signed_s_inv': term}
                if term >= 0.0:
                    payload['production_s_inv'] += term
                    payload['_production_terms'].append(ranked)
                else:
                    payload['loss_s_inv'] += -term
                    payload['_loss_terms'].append(ranked)

            film_growth = 0.0
            if abs(rxn.film_factor) > 0.0 and rxn.surface_id in sys.state_layout.film_index:
                film_growth = float(MONOLAYER_THICKNESS_M * rxn.film_factor * rate / rxn.site_density_m2)
                surface_bucket['film_growth_m_s'] += film_growth

            surface_bucket['reactions'][rxn.reaction_id] = {
                'rate_m2_s': float(rate),
                'net_gas_flux_m2_s': reaction_gas,
                'net_surface_coverage_s_inv': reaction_surface,
                'film_growth_m_s': film_growth,
            }

        for surface_payload in out['surfaces'].values():
            for payload in surface_payload['species'].values():
                prod_terms = sorted(payload.pop('_production_terms'), key=lambda x: x['contribution_s_inv'], reverse=True)
                loss_terms = sorted(payload.pop('_loss_terms'), key=lambda x: x['contribution_s_inv'], reverse=True)
