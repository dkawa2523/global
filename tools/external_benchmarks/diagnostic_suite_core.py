...
def benchmark_by_id(manifest: dict[str, Any], benchmark_id: str) -> dict[str, Any]:
    return _benchmark_by_id(manifest, benchmark_id)
...
def metric_row(
    ...
    *,
    ...
    return _metric_row(manifest, benchmark, metric, value, note=note)
...
def _rate_table_report() -> dict[str, Any]:
    ...
    diagnostics_dict = diagnostics.as_dict() if diagnostics is not None else {}
    clipping_diagnostics_dict = clipping_diagnostics.as_dict() if clipping_diagnostics is not None else {}
    required_diag_keys = {
        'backend',
        'table_path',
        'grid_column',
    ...
    metrics['zdp_table_lookup_diagnostics_present'] = float(required_diag_keys.issubset(diagnostics_dict))
    metrics['zdp_table_clipping_diagnostics_present'] = float(
        bool(clipping_diagnostics_dict.get('lookup_clipped'))
        and bool(clipping_diagnostics_dict.get('lookup_clipped_high'))
        and float(clipping_diagnostics_dict.get('lookup_clipped_value', 0.0)) == float(details['field_max_Td'])
    )
    ...
    details['interpolated_lookup_diagnostics'] = diagnostics_dict
    details['out_of_range_lookup_diagnostics'] = clipping_diagnostics_dict
...
def _facet_for(row: dict[str, Any]) -> str | None:
    ...
    exact = {
        ('CRANE-1', 'species_final'): 'ODE/SI final species',
        ('ZDPlaskin-1', 'species_final'): 'final species',
        ('ZDPlaskin-1', 'circuit_final'): 'circuit final',
        ('PyGMol-1', 'powered_sanity'): 'PyGMol production sanity',
        ('PyGMol-1', 'claim_scope'): 'claim scoping',
    }
    if benchmark == 'PyGMol-Precision-1':
        return 'PyGMol same-footing parity'
    if (benchmark, group) in exact:
        return exact[(benchmark, group)]
    if benchmark == 'ZDPlaskin-1' and group in {'species_transient', 'circuit_transient'}:
        return 'transient / waveform'
    return _facet_for_group_or_category(group, category)


def _facet_for_group_or_category(group: str, category: str) -> str | None:
    group_facets = {
        'conservation': 'charge conservation',
        'schema': 'rate-table schema/grid',
        'grid': 'rate-table schema/grid',
        'diagnostics': 'diagnostics',
        'model_diagnostics': 'diagnostics',
        'backend': 'table backend/interp.',
        'interpolation': 'table backend/interp.',
    }
    if group in group_facets:
        return group_facets[group]
    if category in {'zdplaskin_rate_table', 'swarm_table_ingestion'}:
...
def write_summary(path: Path, manifest: dict[str, Any], rows: list[dict[str, Any]], _figures: list[Path], output_dir: Path) -> None:
